...
func (r *runner) GetDualDCReplicationStatus(ctx context.Context) (bool, error) {
	...
	glog.V(4).Infof("GetDualDCReplicationStatus, res:%v, err:%v", string(res), err)
...
func (r *runner) createDCReplChannel(
	...
	python := fmt.Sprintf(`session.query('CHANGE MASTER TO
		MASTER_HOST="%v",
		MASTER_PORT=%v,
		MASTER_USER="root",
		MASTER_PASSWORD="%v",
	...
	python = fmt.Sprintf(`session.query('CHANGE REPLICATION FILTER
...
func (r *runner) StopDualDCReplication(ctx context.Context, channel string) error {
	python := fmt.Sprintf("session.query('stop slave for channel %s')", channel)
	_, err := r.run(ctx, python)
	if err != nil {
		return err
	}
	python = fmt.Sprintf("session.query('reset slave for channel %s')", channel)
	_, err = r.run(ctx, python)
	return err
}
...
func (r *runner) StartDualDCReplication(
	...
	python := fmt.Sprintf("session.query('start slave for channel %s')", channel)
	_, err = r.run(ctx, python)
	return err
}
