...
type Interface interface {
	...
	GetDualDCReplicationStatus(ctx context.Context) (bool, error)
	...
	StartDualDCReplication(ctx context.Context, host string, port int, passwd string, channel string) error
}
...
func (r *runner) GetDualDCReplicationStatus(ctx context.Context) (bool, error) {
	python := `res = session.query('show slave status').fetch_one(); print (res != None and (res.slave_s_q_l_running == "Yes" or res.slave_i_o_running == "Yes"))`
	res, err := r.run(ctx, python)
	isTrue := string(res) == string(trueBytes)

	glog.V(4).Infof("GetDualDCReplicationStatus, res:%v, res:%v, isTrue:%v, err:%v", string(res), res, isTrue, err)
	return isTrue, err
}
...
func (r *runner) StartDualDCReplication(
	...
	err := r.createDCReplChannel(ctx, host, port, passwd, channel)
	if err != nil {
		glog.V(4).Infof("createDCReplChannel fail: %v", err)
		return err
	}
