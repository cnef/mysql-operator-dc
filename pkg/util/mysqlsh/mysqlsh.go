...
type Interface interface {
	...
	// GetDualDCReplicationStatus check the replication between dual DC
	GetDualDCReplicationStatus(ctx context.Context) (drrepl.ReplicationStatus, error)
	// StopDualDCReplication stop the replication between dual DC
	StopDualDCReplication(ctx context.Context, channel string) error
	// CreateDCReplChannel create the replication channel for dual DC
	CreateDCReplChannel(ctx context.Context, host string, port int, passwd string, channel string) error
	// StartDualDCReplication start the replication between dual DC
	StartDualDCReplication(ctx context.Context, channel string) error
}
...
func (r *runner) GetDualDCReplicationStatus(ctx context.Context) (drrepl.ReplicationStatus, error) {
	const fieldsNum = 4

	python := `res = session.query('show slave status').fetch_one();\
	status = "%s %d %s %d"%(res.slave_i_o_running, res.last_i_o_errno, res.slave_s_q_l_running, res.last_s_q_l_errno) if res != None else "";\
	print status`
	res, err := r.run(ctx, python)

	if err != nil {
		return drrepl.ReplStatusNone, err
	}
	if len(res) <= 1 {
		return drrepl.ReplStatusEmpty, nil
	}
	status := strings.TrimSuffix(string(res), "\n")
	var ioRunning, sqlRunning string
	var ioErrno, sqlErrno int
	nitems, err := fmt.Sscanf(status, "%s %d %s %d", &ioRunning, &ioErrno, &sqlRunning, &sqlErrno)
	if err != nil {
		return drrepl.ReplStatusNone, err
	}
	if nitems != fieldsNum {
		return drrepl.ReplStatusNone, fmt.Errorf("wrong slave status")
	}

	glog.V(4).Infof("GetDualDCReplicationStatus, ioRunning:%v, ioErrno:%v, sqlRunning:%v, sqlErrno:%v",
		ioRunning, ioErrno, sqlRunning, sqlErrno)
	if ioRunning == "No" {
		if ioErrno != 0 {
			return drrepl.ReplStatusIOError, nil
		}
		return drrepl.ReplStatusOFF, nil
	}
	if sqlRunning == "No" {
		if sqlErrno != 0 {
			return drrepl.ReplStatusSQLError, nil
		}
		return drrepl.ReplStatusOFF, nil
	}
	return drrepl.ReplStatusON, nil
}

func (r *runner) CreateDCReplChannel(
	ctx context.Context,
	host string,
	port int,
	passwd string,
	...
	python := fmt.Sprintf(`session.query('CHANGE MASTER TO MASTER_HOST="%v", MASTER_PORT=%v, MASTER_USER="root", MASTER_PASSWORD="%v", MASTER_AUTO_POSITION=1, GET_MASTER_PUBLIC_KEY=1 FOR CHANNEL "%v"')`,
		host,
		port,
		passwd,
		channel)
	...
	python = fmt.Sprintf(`session.query('CHANGE REPLICATION FILTER REPLICATE_IGNORE_DB=(mysql_innodb_cluster_metadata) FOR CHANNEL "%v"')`,
		channel)
	_, err = r.run(ctx, python)
	return err
}

func (r *runner) StopDualDCReplication(ctx context.Context, channel string) error {
	python := fmt.Sprintf(`session.query('STOP SLAVE FOR CHANNEL "%v"')`, channel)
	_, err := r.run(ctx, python)
	if err != nil {
		return err
	}

	python = fmt.Sprintf(`session.query('RESET SLAVE FOR CHANNEL "%v"')`, channel)
	_, err = r.run(ctx, python)
	return err
}

func (r *runner) StartDualDCReplication(
	ctx context.Context,
	channel string,
) error {
	python := fmt.Sprintf(`session.query('START SLAVE FOR CHANNEL "%v"')`, channel)
	_, err := r.run(ctx, python)
	return err
}
